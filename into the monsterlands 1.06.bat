...
set dialogue_pinesvilleguard1=no
set dialogue_pinesvilleguard2=no
...
set pinesvilleguard=present
set ogpinesvilleguard=alive
set firsttimepinesvillegates=no
set times_pinesvilleguardcutscene1_1=0
set times_pinesvilleguardcutscene1_2=0
set times_pinesvilleguardcutscene1_3=0
set times_pinesvilleguardcutscene1_4=0
set times_pinesvilleguardcutscene2_1=0
set times_pinesvilleguardcutscene2_2=0
set times_pinesvilleguardcutscene2_3=0
set times_pinesvilleguardcutscene2_4=0
...
echo         "l" to look around, "i" for inventory, "s" for stats
...
if "%enemy%" == "Pinesville guard" goto combat_pinesvilleguard
)
...
if "%location%" == "pinesvillegates" goto tile_pinesvillegates
...
if "%dialogue_pinesvilleguard1%" == "yes" goto pinesvilleguardcutscene1
if "%dialogue_pinesvilleguard2%" == "yes" goto pinesvilleguardcutscene2
...
echo      1. agility (Affects turn order, escaping, and stealth.)
echo      2. wisdom (Coming soon...)
echo      3. attack (Affects attack accuracy.)
echo      4. strength (Affects attack power and mining.)
echo      5. finesse (Affects critical hit chance, mining, and stealth.)
echo      6. defense (Affects enemy accuracy.)
echo      7. charisma (Affects reputations and social interactions.)
echo.
set input=noInput
set /p input=
...
if "%enemy%" == "Pinesville guard" (
echo    [You examine the Pinesville guard.]
echo      ^> Blue skin, pointy ears, tall. Not to mention, three arms.
echo      ^> An intelligent, capable fighter.
)
...
echo      ^> The rocky cliff here is steep and untraversable. It descends dramatically into the sea far below.
echo      ^> The distant sound of a hundred ocean waves crashing... The beach extends far to the east.
echo      ^> On the southern horizon, the outline of strange foreign buildings. Pinesville.
)
...
if "%location%" == "pinesvillegates" (
echo      ^> A few tall buildings peer over the gates of the walled in village.
echo      ^> Their materials and architexturual style are different from home.
echo      ^> "m" to view map
goto returnLocation
...
echo      ^> It explains everything. "... So that's why we're headed south. Yes... into the monsterlands.
echo      ^> "There are rumors of a place called Pinesville.
...
if "%enemy%" == "Pinesville guard" goto pinesvilleguard_attack
...
if "%enemy%" == "Pinesville guard" goto youwin_pinesvilleguard
...
echo      ^> "i" for inventory, "s" for stats
echo      ^> "e" to escape
echo.
echo    [What do you do?]
echo.
...
echo.
echo    [The goblin shaman does a bonk attack with his little staff.]
set /a attackroll=1 + %RANDOM% %% (20 - 1 + 1)
set /a attackroll=%attackroll% + %enemyattack%
if %attackroll% LSS %AC% (
echo      ^> It misses.
...
:encounter_pinesvilleguard
set combat=yes
set enemy=Pinesville guard
set enemymaxHP_min=18
set enemymaxHP_max=23
set /a enemymaxHP=%enemymaxHP_min% + %RANDOM% %% (%enemymaxHP_max% - %enemymaxHP_min% + 1)
set enemyHP=%enemymaxHP%
set enemydamage_min=2
set enemydamage_max=4
...
set enemyAC=15
set enemyagility=1
:combat_pinesvilleguard
echo.
echo    You are fighting a Pinesville guard (%enemyHP%/%enemymaxHP% HP).
echo      ^> You have %HP%/%maxHP% HP.
if "%confusion%" == "yes" (
echo      ^> You are confused (attack accuracy affected^).
)
...
if %enemyagility% GTR %agility% (
if "%input%" == "1" goto pinesvilleguard_attack
if "%input%" == "a" goto pinesvilleguard_attack
if "%input%" == "attack" goto pinesvilleguard_attack
)
...
if "%input%" == "look at pinesville guard" goto look
if "%input%" == "examine pinesville guard" goto look
if "%input%" == "x pinesville guard" goto look
if "%input%" == "look at guard" goto look
if "%input%" == "examine guard" goto look
if "%input%" == "x guard" goto look
if "%input%" == "x" goto look
...
:pinesvilleguard_attack
set /a enemyspecialmovechance=1 + %RANDOM% %% (10 - 1 + 1)
if %enemyspecialmovechance% LSS 3 goto pinesvilleguard_specialattack

echo.
echo    [The Pinesville guard attacks three times.]
:pinesvilleguard_attack1

set /a attackroll=1 + %RANDOM% %% (20 - 1 + 1)
set /a attackroll=%attackroll% + %enemyattack%
if %attackroll% LSS %AC% (
echo      ^> The first attack misses.
goto pinesvilleguard_attack2
)
set /a damage=%enemydamage_min% + %RANDOM% %% (%enemydamage_max% - %enemydamage_min% + 1)
...
:pinesvilleguard_attack2
set /a attackroll=1 + %RANDOM% %% (20 - 1 + 1)
set /a attackroll=%attackroll% + %enemyattack%
if %attackroll% LSS %AC% (
echo      ^> The second attack misses.
goto pinesvilleguard_attack3
)
set /a damage=%enemydamage_min% + %RANDOM% %% (%enemydamage_max% - %enemydamage_min% + 1)
...
:pinesvilleguard_attack3
set /a attackroll=1 + %RANDOM% %% (20 - 1 + 1)
set /a attackroll=%attackroll% + %enemyattack%
if %attackroll% LSS %AC% (
echo      ^> The third attack misses.
if %agility% == %enemyagility% goto returnCombat
if %agility% GTR %enemyagility% goto returnCombat
if %enemyagility% GTR %agility% goto combat_attack
)
...
set /a critChance=1 + %RANDOM% %% (100 - 1 + 1)
if %critChance% LSS 26 (
set /a damage=%damage% * 2
echo      ^> Critical hit!
)

echo      ^> The third attack deals %damage% damage.
set /a HP=%HP% - %damage%
...
if %enemyagility% GTR %agility% goto combat_attack
goto combat_pinesvilleguard

echo pause 1
pause

:pinesvilleguard_specialattack
set /a HPdeficit=%enemymaxHP% - %enemyHP%
if %HPdeficit% LSS 7 goto pinesvilleguard_attack
echo.
echo    [The Pinesville guard drinks a health potion.]
if %HPdeficit% LSS 10 (
echo      ^> The guard recovers %HPdeficit% HP.
set /a enemyHP=%enemyHP% + %HPdeficit%
)
if %HPdeficit% GTR 9 (
echo      ^> The guard recovers 10 HP.
set /a enemyHP=%enemyHP% + 10
)
goto pinesvilleguard_attack




:youwin_pinesvilleguard
echo.
echo    [You win!]
echo      ^> The Pinesville guard is slain.
echo      ^> You gain %enemymaxHP% exp.
echo      ^> ^+3 iron dirks
echo      ^> ^+1 health potion
echo      ^> "i" for inventory
set /a reputation_pinesville=%reputation_pinesville% - 5
set /a items=%items% + 3
set /a item_irondirk=%item_irondirk% + 1
set /a item_healthpotion=%item_healthpotion% + 2
set ogpinesvilleguard=dead
set pinesvilleguard=absent
set combat=no
set enemy=none
set /a exp=%exp% + %enemymaxHP%
goto checklevelup
...
:cheat1
echo.
echo    [You use cheat 1 for an advantage on your adventures.]
echo      ^> ^+1 attack
echo      ^> ^+5 HP
echo      ^> ^+1 shiny sequined slippers
echo      ^> "i" for inventory
set /a item_shinysequinedslippers=%item_shinysequinedslippers% + 1
set /a attack=%attack% + 1
set /a maxHP=%maxHP% + 5
set HP=%maxHP%
goto returnLocation
...
:cheat3
echo.
echo    [You use cheat 3 to teleport to the cliffs east of the dark fetid cave.]
echo      ^> ^+20 strength
echo      ^> ^+15 attack
echo      ^> ^+30 HP
set /a strength=%strength% + 20
set /a attack=%attack% + 15
set /a maxHP=%maxHP% + 30
set HP=%maxHP%
...
if "%input%" == "cheat1" goto cheat1
if "%input%" == "cheat2" goto cheat2
if "%input%" == "cheat3" goto cheat3
...
if %randomeventchance% GTR 89 goto slimyvinerandomevent
goto returnLocation

:slimyvinerandomevent
...
set location=route3_2
echo.
echo  @ You are in a rocky field near a dark fetid cave.
echo      ^> To the north is a shaky wooden bridge.
echo      ^> To the south is the cave entrance.
echo.
echo    [What do you do?]
echo.
set input=noInput
...
echo. 
echo    [You look around.]
echo      ^> A feeble man pops up from behind the table. It's the apothecary from your village.
...
set location=route3_6
echo.
echo  @ You are at a rocky cliff and the entrance to a dark fetid cave.
echo      ^> To the west is the cave entrance.
echo      ^> (Coming soon: To the east is a sloping sandy beach.
echo      ^> To the south are the gates to Pinesville.
echo.
echo    [What do you do?]
echo.
set input=noInput
...
if "%input%" == "entrance" goto tile_route3_5
if "%input%" == "fetid entrance" goto tile_route3_5
if "%input%" == "dark entrance" goto tile_route3_5
if "%input%" == "dark fetid entrance" goto tile_route3_5
if "%input%" == "go to entrance" goto tile_route3_5
if "%input%" == "go to fetid entrance" goto tile_route3_5
if "%input%" == "go to dark entrance" goto tile_route3_5
if "%input%" == "go to dark fetid entrance" goto tile_route3_5
if "%input%" == "go in entrance" goto tile_route3_5
if "%input%" == "go in fetid entrance" goto tile_route3_5
if "%input%" == "go in dark entrance" goto tile_route3_5
if "%input%" == "go in dark fetid entrance" goto tile_route3_5
if "%input%" == "enter entrance" goto tile_route3_5
if "%input%" == "enter entrance" goto tile_route3_5
if "%input%" == "enter fetid cave entrance" goto tile_route3_5
if "%input%" == "enter dark cave entrance" goto tile_route3_5
if "%input%" == "enter dark entrance" goto tile_route3_5
if "%input%" == "enter dark fetid cave entrance" goto tile_route3_5
if "%input%" == "enter dark fetid entrance" goto tile_route3_5

if "%input%" == "pinesville" goto tile_pinesvillegates
if "%input%" == "go to pinesville" goto tile_pinesvillegates
if "%input%" == "enter pinesville" goto tile_pinesvillegates
if "%input%" == "gates" goto tile_pinesvillegates
if "%input%" == "go to gates" goto tile_pinesvillegates
if "%input%" == "enter gates" goto tile_pinesvillegates
if "%input%" == "pinesville gates" goto tile_pinesvillegates
if "%input%" == "go to pinesville gates" goto tile_pinesvillegates
if "%input%" == "enter pinesville gates" goto tile_pinesvillegates
if "%input%" == "gates of pinesville" goto tile_pinesvillegates
if "%input%" == "go to gates of pinesville" goto tile_pinesvillegates
if "%input%" == "enter gates of pinesville" goto tile_pinesvillegates
if "%input%" == "south" goto tile_pinesvillegates
if "%input%" == "go south" goto tile_pinesvillegates


if "%input%" == "east" goto tile_route4_1
if "%input%" == "go east" goto tile_route4_1
if "%input%" == "go to beach" goto tile_route4_1
if "%input%" == "beach" goto tile_route4_1
if "%input%" == "go to sandy beach" goto tile_route4_1
if "%input%" == "sandy beach" goto tile_route4_1
if "%input%" == "go to sandy sloping beach" goto tile_route4_1
if "%input%" == "sandy sloping beach" goto tile_route4_1
if "%input%" == "go to sloping sandy beach" goto tile_route4_1
if "%input%" == "sloping sandy beach" goto tile_route4_1
...
:tile_pinesvillegates
if "%firsttimepinesvillegates%" == "no" goto pinesvilleguardcutscene1
if "%location%" == "route3_6" (
echo.
echo    [You walk south and approach Pinesville.]
)
set location=pinesvillegates
echo.
echo  @ You are at the gates to Pinesville.
if "%pinesvilleguard%" == "present" (
echo      ^> The Pinesville guard is here.
)
echo      ^> To the north is a rocky coastal cliff.
echo.
echo    [What do you do?]
echo.
set input=noInput
...
if "%input%" == "guard" goto talkto_ogpinesvilleguard
if "%input%" == "talk to guard" goto talkto_ogpinesvilleguard
if "%input%" == "talk to pinesville guard" goto talkto_ogpinesvilleguard
if "%input%" == "talk to her" goto talkto_ogpinesvilleguard
if "%input%" == "talk" goto talkto_ogpinesvilleguard
...
if "%input%" == "north" goto tile_route3_6
if "%input%" == "go north" goto tile_route3_6
if "%input%" == "cliff" goto tile_route3_6
if "%input%" == "go cliff" goto tile_route3_6
if "%input%" == "go to cliff" goto tile_route3_6
if "%input%" == "enter cliff" goto tile_route3_6
if "%input%" == "coastal cliff" goto tile_route3_6
if "%input%" == "go coastal cliff" goto tile_route3_6
if "%input%" == "go to coastal cliff" goto tile_route3_6
if "%input%" == "enter coastal cliff" goto tile_route3_6
if "%input%" == "rocky coastal cliff" goto tile_route3_6
if "%input%" == "go rocky coastal cliff" goto tile_route3_6
if "%input%" == "go to rocky coastal cliff" goto tile_route3_6
if "%input%" == "enter rocky coastal cliff" goto tile_route3_6
if "%input%" == "rocky cliff" goto tile_route3_6
if "%input%" == "go rocky cliff" goto tile_route3_6
if "%input%" == "go to rocky cliff" goto tile_route3_6
if "%input%" == "enter rocky cliff" goto tile_route3_6

if "%input%" == "south" goto tile_pinesville_A
if "%input%" == "go south" goto tile_pinesville_A

if "%input%" == "enter" goto tile_pinesville_A
if "%input%" == "enter gates" goto tile_pinesville_A
if "%input%" == "go to gates" goto tile_pinesville_A
if "%input%" == "enter village" goto tile_pinesville_A
if "%input%" == "go to village" goto tile_pinesville_A
if "%input%" == "enter pinesville" goto tile_pinesville_A
if "%input%" == "go to pinesville" goto tile_pinesville_A
if "%input%" == "enter village gates" goto tile_pinesville_A
if "%input%" == "go to village gates" goto tile_pinesville_A
if "%input%" == "enter pinesville gates" goto tile_pinesville_A
if "%input%" == "go to pinesville gates" goto tile_pinesville_A

goto inventoryCommands



:talkto_ogpinesvilleguard
set location=pinesvillegates
echo.
echo    [You talk to the Pinesville guard.]
echo      ^> She has blue skin, long pointy ears, a tall stature, and three arms.
echo      ^> "My orders are firm," she says. "Go back north to the beach with the rest."
echo.
echo    [What do you say?]
echo.
if "%pinesvilleguardcutscene1_1%" LSS 1 (
echo      1. Why isn't Pinesville taking any refugees?
)
if "%pinesvilleguardcutscene1_2%" LSS 1 (
echo      2. Have you seen my best friend?
)
if "%pinesvilleguardcutscene1_3%" LSS 1 (
echo      3. I never knew there were other people out here in the monsterlands.
)
if "%pinesvilleguardcutscene1_4%" LSS 1 (
echo      4. Haha, you look funny.
)
if "%pinesvilleguardcutscene2_1%" LSS 1 (
echo      5. I came a long way alone to get here. Please make an exception for me. (Persuade.^)
)
if "%pinesvilleguardcutscene2_2%" LSS 1 (
echo      6. I'm entering Pinesville and you can't stop me. (Fight.^)
)
if "%pinesvilleguardcutscene2_3%" LSS 1 (
echo      7. What's that over by that tree? (Sneak.^)
)
if "%pinesvilleguardcutscene2_4%" LSS 1 (
echo      8. Okay, I'll go somewhere else. (Leave.^)
)
echo.
set input=noInput
set /p input=
...
if "%input%" == "1" goto pinesvilleguardcutscene1_1
if "%input%" == "2" goto pinesvilleguardcutscene1_2
if "%input%" == "3" goto pinesvilleguardcutscene1_3
if "%input%" == "4" goto pinesvilleguardcutscene1_4
if "%input%" == "5" goto pinesvilleguardcutscene2_1
if "%input%" == "6" goto pinesvilleguardcutscene2_2
if "%input%" == "7" goto pinesvilleguardcutscene2_3
if "%input%" == "8" goto pinesvilleguardcutscene2_4
if "%input%" == "persuade" goto pinesvilleguardcutscene2_1
if "%input%" == "fight" goto pinesvilleguardcutscene2_2
if "%input%" == "sneak" goto pinesvilleguardcutscene2_3
if "%input%" == "leave" goto pinesvilleguardcutscene2_4
...
:pinesvilleguardcutscene1
set location=pinesvillegates
set firsttimepinesvillegates=yes
echo.
echo    [You approach the closed gates of Pinesville.]
echo      ^> A guard here slays a cave fly with a flourish and then sheathes her three weapons.
echo      ^> "Look what we have here," she says. "Another peaceful little villager from up north."
echo      ^> You have never seen a person from Pinesville before.
echo      ^> The guard has blue skin, long pointy ears, a tall stature, and three arms.
echo      ^> "Pinesville is closed off. Go back north to the beach with the rest of the refugees."
echo.
echo    [What do you say?]
echo.
echo      1. Why isn't Pinesville taking any refugees?
...
:pinesvilleguardcutscene1_1

set /a times_pinesvilleguardcutscene1_1=%times_pinesvilleguardcutscene1_1% + 1
echo.
echo    [You ask why Pinesville isn't taking any refugees.]
echo      ^> "So at least some of you people do care to know about our perspective...
echo      ^> "Would you have opened your village up to random strangers?
echo      ^> "The changes in the monsterlands have created enough problems for us already."
set /a reputation_pinesville=%reputation_pinesville% + 1
goto pinesvilleguardcutscene2

:pinesvilleguardcutscene1_2
set /a times_pinesvilleguardcutscene1_2=%times_pinesvilleguardcutscene1_2% + 1
echo.
echo    [You describe your friend.]
if %charisma% GTR 1 (
echo      ^> The guard's arms unfold for a moment. "I actually did see someone like that.
echo      ^> "Your friend was definitely among the villagers who went back north from here to the beach."
echo      ^> She points northeast.
)
if %charisma% LSS 2 (
echo      ^> "I don't know." The guard doesn't even seem to think about it.
set reputation_pinesville=%reputation_pinesville% - 1
)
goto pinesvilleguardcutscene2

:pinesvilleguardcutscene1_3
set /a times_pinesvilleguardcutscene1_3=%times_pinesvilleguardcutscene1_3% + 1
echo.
echo    [You marvel at your differences.]
echo      ^> "Yes, we all look like this," the guard says.
echo      ^> "And we hardly knew of your peaceful village either." She is looking down at you.
echo      ^> "Rarely have we so much as encountered the goblins north of that dark fetid cave."
goto pinesvilleguardcutscene2



:pinesvilleguardcutscene1_4
set /a times_pinesvilleguardcutscene1_4=%times_pinesvilleguardcutscene1_4% + 1
echo.
echo    [You laugh at the guard.]
echo      ^> She simply frowns, unamused.
set reputation_pinesville=%reputation_pinesville% - 1
goto pinesvilleguardcutscene2
...
:pinesvilleguardcutscene2
echo      ^> "Anyways, my orders are firm. You're not allowed in."
echo      ^> She crosses all her arms again like a pretzel.
echo.
echo    [What do you say?]
echo.
echo      1. I came a long way alone to get here. Please make an exception for me. (Persuade.^)
echo      2. I'm entering Pinesville and you can't stop me. (Fight.^)
echo      3. What's that over by that tree? (Sneak.^)
echo      4. Okay, I'll go somewhere else. (Leave.^)
echo.
set input=noInput
set /p input=
if "%input%" == "noInput" goto returnLocation
if "%input%" == "1" goto pinesvilleguardcutscene2_1
if "%input%" == "2" goto pinesvilleguardcutscene2_2
if "%input%" == "3" goto pinesvilleguardcutscene2_3
if "%input%" == "4" goto pinesvilleguardcutscene2_4
if "%input%" == "persuade" goto pinesvilleguardcutscene2_1
if "%input%" == "fight" goto pinesvilleguardcutscene2_2
if "%input%" == "sneak" goto pinesvilleguardcutscene2_3
if "%input%" == "leave" goto pinesvilleguardcutscene2_4
goto commandErrormmandError



:pinesvilleguardcutscene2_1
set /a times_pinesvilleguardcutscene2_1=%times_pinesvilleguardcutscene2_1% + 1
echo.
echo    [You attempt to persuade the guard.]
if %charisma% GTR 1 (
echo      ^> "You really don't seem half bad," she says, "but I have my orders."
echo      ^> She leans forward. "I'll tell you this, even though I have no reason to.
echo      ^> "A boy from Pinesville disappeared. He was last seen playing up near the bridge.
echo      ^> "I bet if you found the kid, Pinesville would be more accepting of your people."
)
if %charisma% GTR 2 (
echo      ^> The guard looks both ways, then hands you a vile. "You may need this."
echo      ^> ^+1 antidote
echo      ^> "i" for inventory
set /a item_antidote=%item_antidote% + 1
)
if %charisma% LSS 2 (
echo      ^> She hovers one of her three weapons, two arms still folded. "No means no."
)
goto pinesvilleguardcutscene2

:pinesvilleguardcutscene2_2
set /a times_pinesvilleguardcutscene2_2=%times_pinesvilleguardcutscene2_2% + 1
echo.
echo    [You challenge the guard.]
set location=pinesvillegates
goto encounter_pinesvilleguard


:pinesvilleguardcutscene2_3
set /a times_pinesvilleguardcutscene2_3=%times_pinesvilleguardcutscene2_3% + 1
echo.
echo    [You try to trick the guard.]
if %charisma% LSS 1 (
echo      ^> She doesn't buy it for a second. She just frowns back at you.
set /a reputation_pinesville=%reputation_pinesville% - 1
goto tile_pinesvillegates
)
set /a stealth=%agility% + %finesse%
if %stealth% GTR 2 (
echo      ^> She looks away for a second. "Huh?"
echo      ^> You slink through the gates. They shut silently behind you.
goto tile_pinesville_A
)
echo      ^> She looks away but not long enough.
echo      ^> "And just where do you think you're going?" She blocks you with a sword.
set /a reputation_pinesville=%reputation_pinesville% - 1
goto tile_pinesvillegates

:pinesvilleguardcutscene2_4
set /a times_pinesvilleguardcutscene2_4=%times_pinesvilleguardcutscene2_4% + 1
echo.
echo    [You finish talking to the guard.]
if %reputation_pinesville% LSS -2 (
echo      ^> "Mhmm, now get lost."
goto tile_pinesvillegates
)

if %reputation_pinesville% LSS 3 (
echo      ^> She nods a farewell.
goto tile_pinesvillegates
)

if %reputation_pinesville% LSS 9 (
echo      ^> "Take care."
goto tile_pinesvillegates
)

if %reputation_pinesville% LSS GTR 8 (
...
if "%location%" == "pinesvillegates" (
echo.
echo    [You enter the gates of Pineville.]
)
if "%location%" == "pinesville_B" (
echo.
echo    [You walk north, out of the center of Pinesville.]
)
set location=pinesville_A
echo.
echo  @ You are in Pinesville, on the north side.
echo      ^> The city extends far to the south.
echo      ^> To the north are the city gates.
echo.
echo    [What do you do?]
echo.
set input=noInput
...
if "%input%" == "north" goto tile_pinesvillegates
if "%input%" == "go north" goto tile_pinesvillegates
if "%input%" == "exit" goto tile_pinesvillegates
if "%input%" == "leave" goto tile_pinesvillegates
if "%input%" == "go to gates" goto tile_pinesvillegates
if "%input%" == "exit gates" goto tile_pinesvillegates
if "%input%" == "go to city gates" goto tile_pinesvillegates
if "%input%" == "exit city gates" goto tile_pinesvillegates
if "%input%" == "exit city" goto tile_pinesvillegates
if "%input%" == "exit pinesville" goto tile_pinesvillegates

if "%input%" == "south" goto tile_pinesville_B
if "%input%" == "go south" goto tile_pinesville_B
...
:tile_pinesville_B
if "%location%" == "pinesville_A" (
echo.
echo    [You walk south into the center of Pinesville.]
)
if "%location%" == "pinesville_C" (
echo.
echo    [You walk north into the center of Pinesville.]
)
set location=pinesville_B
echo.
echo  @ You are in the center of Pinesville.
echo      ^> The city extends to the north and south.
echo.
echo    [What do you do?]
echo.
set input=noInput
...
if "%input%" == "south" goto tile_pinesville_C
if "%input%" == "go south" goto tile_pinesville_C
if "%input%" == "north" goto tile_pinesville_A
if "%input%" == "go north" goto tile_pinesville_A
...
if "%location%" == "pinesville_B" (
echo.
echo    [You walk south, out of the center of Pinesville.]
)
set location=pinesville_C
echo.
echo  @ You are in Pinesville, on the south side.
echo      ^> The city extends far to the north.
echo.
echo    [What do you do?]
echo.
set input=noInput
...
if "%input%" == "north" goto tile_pinesville_B
if "%input%" == "go north" goto tile_pinesville_B
...
if "%location%" == "route3_6" (
echo.
echo    [You walk east down the sloping shore to the beach.]
)
if "%location%" == "route4_2" (
echo.
echo    [You walk west up the sloping shore from the beach.]
)
set location=route4_1
echo.
echo  @ You are on a sloping sandy beach.
echo      ^> To the west is the entrance to a dark fetid cave.
echo      ^> The beach extends to the east.
echo.
echo    [What do you do?]
echo.
set input=noInput
...
if "%input%" == "cave" goto tile_route3_6
if "%input%" == "go to cave" goto tile_route3_6
if "%input%" == "cave entrance" goto tile_route3_6
if "%input%" == "go to cave entrance" goto tile_route3_6
if "%input%" == "electrical cave" goto tile_route3_6
if "%input%" == "go to electrical cave" goto tile_route3_6
if "%input%" == "electrical cave entrance" goto tile_route3_6
if "%input%" == "go to electrical cave entrance" goto tile_route3_6
if "%input%" == "glowing electrical cave" goto tile_route3_6
if "%input%" == "go to glowing electrical cave" goto tile_route3_6
if "%input%" == "glowing electrical cave entrance" goto tile_route3_6
if "%input%" == "go to glowing electrical cave entrance" goto tile_route3_6
if "%input%" == "glowing cave" goto tile_route3_6
if "%input%" == "go to glowing cave" goto tile_route3_6
if "%input%" == "glowing cave entrance" goto tile_route3_6
if "%input%" == "go to glowing cave entrance" goto tile_route3_6

if "%input%" == "west" goto tile_route3_6
if "%input%" == "go west" goto tile_route3_6
if "%input%" == "east" goto tile_route4_2
if "%input%" == "go east" goto tile_route4_2
...
:tile_route4_2
if "%location%" == "route4_1" (
echo.
echo    [You walk east down the sloping shore along the beach.]
)
if "%location%" == "route4_3" (
echo.
echo    [You walk west up the sloping shore.]
)
set location=route4_1
echo.
echo  @ You are on a sloping sandy beach.
echo      ^> The beach extends to the west.
echo      ^> To the east is the entrance to a glowing electrical cave.
echo.
echo    [What do you do?]
echo.
set input=noInput
...
if "%input%" == "west" goto tile_route4_1
if "%input%" == "go west" goto tile_route4_1

if "%input%" == "east" goto tile_route4_3
if "%input%" == "go east" goto tile_route4_3
if "%input%" == "cave" goto tile_route4_3
if "%input%" == "go to cave" goto tile_route4_3
if "%input%" == "cave entrance" goto tile_route4_3
if "%input%" == "go to cave entrance" goto tile_route4_3
if "%input%" == "electrical cave" goto tile_route4_3
if "%input%" == "go to electrical cave" goto tile_route4_3
if "%input%" == "electrical cave entrance" goto tile_route4_3
if "%input%" == "go to electrical cave entrance" goto tile_route4_3
if "%input%" == "glowing electrical cave" goto tile_route4_3
if "%input%" == "go to glowing electrical cave" goto tile_route4_3
if "%input%" == "glowing electrical cave entrance" goto tile_route4_3
if "%input%" == "go to glowing electrical cave entrance" goto tile_route4_3
if "%input%" == "glowing cave" goto tile_route4_3
if "%input%" == "go to glowing cave" goto tile_route4_3
if "%input%" == "glowing cave entrance" goto tile_route4_3
if "%input%" == "go to glowing cave entrance" goto tile_route4_3
...
if "%location%" == "route4_2" (
echo.
echo    [You walk east down the sloping shore to a cave entrance.]
)
if "%location%" == "route4_4" (
echo.
echo    [You walk west and exit the glowing electrical cave.]
)
set location=route4_1
echo.
echo  @ You are at the entrance to a glowing electrical cave.
echo      ^> To the west is a sloping sandy beach.
echo      ^> To the east is the inside of the electrical cave.
echo.
echo    [What do you do?]
echo.
set input=noInput
...
if "%input%" == "west" goto tile_route4_2
if "%input%" == "go west" goto tile_route4_2
if "%input%" == "beach" goto tile_route4_2
if "%input%" == "go to beach" goto tile_route4_2
if "%input%" == "sandy beach" goto tile_route4_2
if "%input%" == "go to sandy beach" goto tile_route4_2
if "%input%" == "sloping sandy beach" goto tile_route4_2
if "%input%" == "go to sloping sandy beach" goto tile_route4_2
if "%input%" == "sloping beach" goto tile_route4_2
if "%input%" == "go to sloping beach" goto tile_route4_2


if "%input%" == "east" goto tile_route4_4
if "%input%" == "go east" goto tile_route4_4
if "%input%" == "cave" goto tile_route4_4
if "%input%" == "go to cave" goto tile_route4_4
if "%input%" == "cave entrance" goto tile_route4_4
if "%input%" == "go to cave entrance" goto tile_route4_4
if "%input%" == "electrical cave" goto tile_route4_4
if "%input%" == "go to electrical cave" goto tile_route4_4
if "%input%" == "electrical cave entrance" goto tile_route4_4
if "%input%" == "go to electrical cave entrance" goto tile_route4_4
if "%input%" == "glowing electrical cave" goto tile_route4_4
if "%input%" == "go to glowing electrical cave" goto tile_route4_4
if "%input%" == "glowing electrical cave entrance" goto tile_route4_4
if "%input%" == "go to glowing electrical cave entrance" goto tile_route4_4
if "%input%" == "glowing cave" goto tile_route4_4
if "%input%" == "go to glowing cave" goto tile_route4_4
if "%input%" == "glowing cave entrance" goto tile_route4_4
if "%input%" == "go to glowing cave entrance" goto tile_route4_4
if "%input%" == "enter cave" goto tile_route4_4
if "%input%" == "enter cave entrance" goto tile_route4_4
if "%input%" == "enter electrical cave" goto tile_route4_4
if "%input%" == "enter electrical cave entrance" goto tile_route4_4
if "%input%" == "enter glowing electrical cave" goto tile_route4_4
if "%input%" == "enter glowing electrical cave entrance" goto tile_route4_4
if "%input%" == "enter glowing cave" goto tile_route4_4
if "%input%" == "enter glowing cave entrance" goto tile_route4_4
if "%input%" == "go in cave" goto tile_route4_4
if "%input%" == "go in cave entrance" goto tile_route4_4
if "%input%" == "go in electrical cave" goto tile_route4_4
if "%input%" == "go in electrical cave entrance" goto tile_route4_4
if "%input%" == "go in glowing electrical cave" goto tile_route4_4
if "%input%" == "go in glowing electrical cave entrance" goto tile_route4_4
if "%input%" == "go in glowing cave" goto tile_route4_4
if "%input%" == "go in glowing cave entrance" goto tile_route4_4
if "%input%" == "go into cave" goto tile_route4_4
if "%input%" == "go into cave entrance" goto tile_route4_4
if "%input%" == "go into electrical cave" goto tile_route4_4
if "%input%" == "go into electrical cave entrance" goto tile_route4_4
if "%input%" == "go into glowing electrical cave" goto tile_route4_4
if "%input%" == "go into glowing electrical cave entrance" goto tile_route4_4
if "%input%" == "go into glowing cave" goto tile_route4_4
if "%input%" == "go into glowing cave entrance" goto tile_route4_4
if "%input%" == "go inside cave" goto tile_route4_4
if "%input%" == "go inside cave entrance" goto tile_route4_4
if "%input%" == "go inside electrical cave" goto tile_route4_4
if "%input%" == "go inside electrical cave entrance" goto tile_route4_4
if "%input%" == "go inside glowing electrical cave" goto tile_route4_4
if "%input%" == "go inside glowing electrical cave entrance" goto tile_route4_4
if "%input%" == "go inside glowing cave" goto tile_route4_4
if "%input%" == "go inside glowing cave entrance" goto tile_route4_4
...
:tile_route4_4
echo.
echo    [You move to enter the cave.]
echo      ^> You cannot pass. The entrance is caved in.
goto tile_route4_3